...
const urlOpenWeather = `https://api.openweathermap.org/data/3.0/onecall?lat=${lat}&lon=${lon}&units=metric&exclude={hourly,minutely,alerts}&appid=${apiKey}`;
...
function displayWeatherResults(data) {
    ...
    feelsLike.innerHTML = `${data.main.feels_like.toFixed(0)}&deg;C`;
...
function displayOneCallResults(data) {
    ...
    dailyResults.forEach((day) => {
        ...
        maxTempT.textContent = `High:`;
        maxTempDiv.appendChild(maxTemp);
        maxTemp.innerHTML = `${day.temp.max.toFixed(0)}&deg;C`;

        weatherDay.appendChild(minTempDiv);
        minTempDiv.appendChild(minTempT);
        minTempT.textContent = `Low:`;
        minTempDiv.appendChild(minTemp);
        minTemp.innerHTML = `${day.temp.min.toFixed(0)}&deg;C`;
